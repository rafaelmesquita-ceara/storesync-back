...
### 2. Escrever os testes (se ainda não existirem)
- Analise o que foi implementado e escreva testes unitários correspondentes nos arquivos `StoreSyncBack.Tests/Unit/Services/`
- Siga o padrão: `[Metodo]_[Estado]_[ResultadoEsperado]`
- Use `TestData` para dados fake e `Moq` para mocks
- Cubra os cenários: caminho feliz + casos de erro relevantes

### 3. Executar os testes e confirmar que passam (verde)
```bash
export DOTNET_ROOT="/opt/homebrew/opt/dotnet@9/libexec"
export PATH="/opt/homebrew/opt/dotnet@9/bin:$PATH"
